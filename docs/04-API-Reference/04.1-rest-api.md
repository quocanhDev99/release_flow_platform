# REST API Reference (Release Flow Platform)
...
## Deployment Windows (Schedules & Cron Jobs)
* **GET `/deployment-windows`**: Retrieves all deployment schedules.
* **POST `/deployment-windows`**: Creates a new deployment schedule.
* **PUT `/deployment-windows/{id}`**: Updates schedule details (Build Environment, Build Time, Fix Version).
* **DELETE `/deployment-windows/{id}`**: Deletes a deployment schedule.
* **GET `/deployment-windows/{id}/release-notes`**: Generates and retrieves structured release notes, Markdown text, and responsive HTML email markup for a specific deployment window.
* **POST `/deployment-windows/{id}/release-notes/broadcast`**: Broadcasts the release notes and handover report to selected channels (`telegram`, `teams`, `slack`, `email`).
* **POST `/deployment-windows/clear-month`**: Deletes all schedules within a specific month (used prior to AI OCR sync).
* **POST `/deployment-windows/notify`**: Triggers real-time notification alert for schedule changes.
* **POST `/deployment-windows/trigger-reminder`**: Triggers daily deployment reminder broadcast.
* **GET `/deployment-windows/cron/status`**: Retrieves active automated cron job metadata (job IDs, schedule times, channels, active status).
* **POST `/deployment-windows/cron/run/{jobId}`**: Triggers an automated cron job on demand (`jobId`: `daily_reminder` or `tomorrow_reminder`).
...
* **POST `/users/login`**: Authenticates user credentials and returns the user profile.
