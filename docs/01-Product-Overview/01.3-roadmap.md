...
## 🔹 Version 2.8: Auto Release Notes & Handover Studio (Completed)
*Focus: Automate post-deployment communication, release notes drafting, and multi-channel team handovers.*

*   **Intelligent Scope Aggregator**: Automatically resolves all Jira tickets, SOW scopes, PR commit authors, and affected Git repositories for any deployment window.
*   **Multi-Format Export**:
    *   *Visual Handover*: Interactive KPI cards (Total Tickets, Features, Fixes, Improvements) with QA verification checklist.
    *   *Markdown*: GitHub/Confluence flavored markdown with 1-click clipboard copy and `.md` file download.
    *   *Rich HTML Email*: Responsive email template with styled table and status badges, copyable directly into Microsoft Outlook and Gmail via `ClipboardItem` API.
*   **1-Click Multi-Channel Broadcast**: Instant dispatching of the handover report to Telegram, Microsoft Teams, Slack, and SMTP Email lists simultaneously.
